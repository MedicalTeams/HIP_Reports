...
/****** Object:  StoredProcedure [dbo].[sp35_Chronic_Disease]    Script Date: 2/5/2018 1:19:13 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROC [dbo].[sp35_Chronic_Disease]
...
SET @End_Date= DATEADD(ns, -100, DATEADD(s, 86400, @End_Date))
	...
	SELECT
		...
		,X.Supplemental_Chronic_Disease_Category
		,X.bnfcry
		,X.bnfcry_id
		,X.gndr_cd
		,X.gndr_id
	...
	END AS Supplemental_Chronic_Disease_Category
	...
	WHERE
		...
		AND LKDX.diag_id = 19 --Chronic Disease
		...
		--AND LKDX.diag_descn = 'Chronic Disease'
	) AS x
	...
	ORDER BY 1, 2, 3, 4, 5, 6, 7 DESC, 8 DESC, 9, 10
